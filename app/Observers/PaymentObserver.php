    ...
    public function created(Payment $payment): void
    {
        ...
        try {
            ...
            foreach ($user_super_admins as $admin) {
                $data[] = [
                    'to' => env('MAIL_ENVIRONMENT') == 'production' ? whatsappNumber($admin->phone) : whatsappNumber(env('WHATSAPP_ADMIN_NUMBER')),
                    'text' => "Halo Bapak/Ibu " . $admin->name . ",\n\n" .
                        "🔔 Ada Pembayaran Masuk dari Rumah Jurnal. " .
            ...
            if ($response->status() != 200) {
                Log::error('Failed to send admin notification for payment ' . $payment->id . '. Response: ' . $response->body());
            } else {
                Log::info('Admin notification sent for payment ' . $payment->id);
            }
        } catch (\Exception $e) {
