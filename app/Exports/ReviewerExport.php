...
class ReviewerExport implements WithStyles, WithEvents
{
    protected $issue_id;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'Reviewer Report');
                ...
                $sheet->setCellValue('A6', 'Total Reviewer: ' . $issue->reviewers->count());
                ...
                $sheet->setCellValue('B8', 'Nama Reviewer');
                ...
                foreach ($reviewers as $index => $reviewer) {
                    $titles = $reviewer->submissionsReviewed->pluck('submission.fullTitle')->toArray();
                    ...
                    for ($i = 0; $i < $articleCount; $i++) {
                        ...
                        if ($i === 0) {
                            ...
                            $sheet->setCellValue('B' . $row, $reviewer->name);
                            $sheet->setCellValue('C' . $row, $reviewer->affiliation);
                            $sheet->setCellValue('D' . $row, $articleCount);
                            $sheet->setCellValue('F' . $row, "Reviewer");
                        }
                    ...
                    if ($articleCount === 0) {
                        ...
                        $sheet->setCellValue('B' . $startRow, $reviewer->name);
                        $sheet->setCellValue('C' . $startRow, $reviewer->affiliation);
                        $sheet->setCellValue('D' . $startRow, 0);
                        $sheet->setCellValue('E' . $startRow, '-');
                        $sheet->setCellValue('F' . $startRow, "Reviewer");
